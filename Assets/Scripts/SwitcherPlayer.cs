...
public class PlayerSwitcher : MonoBehaviour
{
    ...
    public void OnBuyButton()
    {
        if (!CharacterManager.IsPurchased(data[currentIndex].id)&& PlayerPrefs.GetInt("Coin", 0)>= data[currentIndex].price)
        {
            ...
            UpdateUI();
            NotificationManger.Instance.ShowNotification("You have unlocked a new character", 2f);
        }
        else
        {
            NotificationManger.Instance.ShowNotification("You don't have enough money", 2f);
        }
    }
